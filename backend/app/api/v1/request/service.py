...
def get_my_requests(session: Session, member_id: int | None) -> list[RequestItem]:
...
def create_request(session: Session, data: CreateRequest) -> int:
...
def get_activities(
    session: Session, start: date, end: date, statuses: list[str], businesses: list[int]
) -> list[Activity]:
    sql = text(
...
def get_home_metrics(session: Session, start: date, end: date):
    rows = session.execute(
        text(
...
def get_tracking(
    session: Session, start: date, end: date, business: int | None, category: int | None
):
    p = {
        "start": start,
        "end": end,
    ...
    businesses = session.execute(
        text("SELECT id,name FROM business ORDER BY name")
    ).mappings()
    categories = session.execute(
        text("SELECT id,name FROM service_category ORDER BY name")
    ).mappings()
...
def get_board(session: Session, start: date, end: date):
    statuses = [
        dict(r)
        for r in session.execute(
            text("SELECT id,description FROM request_status ORDER BY id")
        ).mappings()
    ]
    rows = session.execute(
        text(
    ...
    for r in rows:
        ...
        for t in tasks:
            ...
            checklists = []
            for checklist in session.execute(
                text("""SELECT rtc.id, ct.id checklist_type_id, ct.name, ct.description,
